-- ============================================================
-- NeuroNest — Full Supabase Schema (v4)
-- No doctor, no community. Users only.
...
-- role is always 'user' — no doctor role.
...
-- Used to calculate: streak, wellness score, mood chart, monthly check-in count.
create table public.mood_logs (
  id         uuid primary key default gen_random_uuid(),
  user_id    uuid not null references auth.users(id) on delete cascade,
  mood       text not null,        -- emoji e.g. "😄"
  mood_label text not null,        -- text  e.g. "Great"
...
-- ── 6. chat_sessions ─────────────────────────────────────────────────────────
-- One row per conversation. Title auto-updates to first message text.
...
-- role is either 'user' (typed by the person) or 'ai' (NeuroBot reply).
...
-- Runs as security definer (superuser privileges) so it can insert into
-- profiles even though the user's session doesn't exist yet at trigger time.
-- $$ dollar-quoting is required — single $ will break the function body.
