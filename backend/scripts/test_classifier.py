...
# PROFILES mapped exactly to the TS implementation weights & biases
PROFILES = {
    "happy": {
        "features": {
...
def run_tests():
    ...
    print("--- Evaluating current classifier rules on reference images ---")
    correct = 0
    for expected_emotion, path in test_cases.items():
        if not os.path.exists(path):
            print(f"Skipping {expected_emotion}: file not found {path}")
            continue
